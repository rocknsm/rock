...
with open('tests/vars/fsf.vars', 'r') as f:
    try:
...
def test_fsf_client_to_server(host):
    results = host.run('/opt/fsf/fsf-client/fsf_client.py /usr/bin/cd')
    if 'socket.error' in results.stdout:
        assert False
    else:
        assert True
