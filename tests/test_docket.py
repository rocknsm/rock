...
with open('tests/vars/docket.vars', 'r') as f:
    try:
...
@pytest.mark.parametrize("port", yml_vars.get('firewall_ports'))
# test to see if port is allowed through firewalld
def test_firewall_rules(host, port):
    with host.sudo():
        out = host.check_output("firewall-cmd {}".format('--list-ports'))
...
def test_network_sockets(host, port):
    ...
    if(port != 6379):
        assert host.run_test('curl -k https://{host}:{p}'.format(host='localhost', p=port))
